...
public class QueryBenchmarks
{
    ...
    private Dictionary<string, List<Interval<long, int>>> _dataCache = new();
    ...
    public object GetLoadedTree(string treeType, string dataType)
    {
        ...
        {
            var tree = new IntervalTree.IntervalTree<long, long>();
        ...
        {
            IIntervalTree<long, long> tree = treeType switch
            {
                "light" => new LightIntervalTree<long, long>(),
                "lightopt" => new LightIntervalTreeOpt<long, long>(),
                "quick" => new QuickIntervalTree<long, long>(),
                _ => throw new ArgumentException($"Unkown tree type: {treeType}")
            };
    ...
    {
        var tree = (IntervalTree.IntervalTree<long, long>)GetLoadedTree("reference", DataType);
    ...
    {
        var tree = (LightIntervalTree<long, long>)GetLoadedTree("light", DataType);
        for (var i = 0; i < 10_000; i++)
        {
            var resultCount = 0;
            var results = tree.Query(i);
    ...
    [Benchmark(OperationsPerInvoke = 10_000)]
    public void QueryLightOpt()
    {
        var tree = (LightIntervalTreeOpt<long, long>)GetLoadedTree("lightopt", DataType);
        for (var i = 0; i < 10_000; i++)
        {
            var resultCount = 0;
            var results = tree.Query(i);
    ...
    {
        var tree = (QuickIntervalTree<long, long>)GetLoadedTree("quick", DataType);
        for (var i = 0; i < 10_000; i++)
        {
            var resultCount = 0;
            var results = tree.Query(i);
