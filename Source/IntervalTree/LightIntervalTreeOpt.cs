using Jamarino.ValueEnumerable;
using System.Collections;

namespace Jamarino.IntervalTree;

public class LightIntervalTreeOpt<TKey, TValue> : IIntervalTree<TKey, TValue>
    where TKey : IComparable<TKey>
{
    ...
    IEnumerable<TValue> IIntervalTree<TKey, TValue>.Query(TKey target)
    {
        return Query(target);
    }

    public EmptyOrEnumerable<TValue> Query(TKey target)
    {
        if (_isBuilt is false)
            Build();

        if (_treeSizeFull is 0)
            return new EmptyOrEnumerable<TValue>();

        var results = new EmptyOrEnumerable<TValue>();
